				...
				<tr>
					...
					<td class="td_table_2" colspan="3">
						<c:forEach var="khcs" items="${khcss}">
							<input type="radio" name="khcsId" class="${khcs.score}" value="${khcs.id}" readonly <c:if test="${dwcl.khcsId == khcs.id }">checked</c:if>/>${khcs.khxm}</br>
						</c:forEach>
					</td>
				</tr>
			</table>
			...
			<table class="table_all" align="center" border="0" cellpadding="0"
				...
				<tr>
					<td class="td_table_2" style="height:50px" rowspan="2" >
						<textarea style="width:100%;height:100%" name="suggest" readonly>${dwcl.suggest }</textarea>
					</td>
					<td class="td_table_2">
						<frame:select name="spzt" type="radio" configName="spzt" value="${dwcl.spzt}" cssClass="input_select"/>
					</td>
				</tr>
				<%--<tr align="left">
