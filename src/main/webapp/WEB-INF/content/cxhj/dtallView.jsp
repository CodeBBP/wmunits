			...
			$.ajax( {
				...
				success : function(data) {
					for ( var i = 0; i < data.length; i++) {
						addMarker(data[i].xpoint, data[i].ypoint,data[i].title,data[i].sjzp,data[i].content);
					}
				}
			});
		});
...
// 编写自定义函数,创建标注
function addMarker(xpoint, ypoint, title, image,content) {
	var point = new BMap.Point(xpoint, ypoint);
	var marker = new BMap.Marker(point);
	var sContent = "<h4 style='margin:0 0 5px 0;padding:0.2em 0'>"+title+"</h4>"
			+ "<img style='float:right;margin:4px' id='imgDemo' src='"+image+"' width='139' height='104' title='街道'/>"
