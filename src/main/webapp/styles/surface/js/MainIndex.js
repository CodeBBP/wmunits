...
function Maximize() {
    $("#Header").hide();
    $("#full_screen").attr('src', '/wmdw/styles/surface/images/arrow_out.gif').attr('title', '还原').attr('onclick', 'Fullrestore()');
...
function Fullrestore() {
    $("#Header").show();
    $("#full_screen").attr('src', '/wmdw/styles/surface/images/arrow_out.gif').attr('title', '最大化').attr('onclick', 'Maximize()');
    MainContent_subtract = MainContent_subtract + 70;
    Sidebar_subtract = Sidebar_subtract + 70;
    iframeresize();
}
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace) {
    ...
    if (document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','" + img + "','true')\"><a href=\"javascript:;\"><img src=\"/wmdw/styles/surface/images/" + img + "\" width=\"20\" height=\"20\">" + name + "</a></span><a title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\">×</a></li>");
        } else {
        ...
        document.getElementById("tabs_iframe_" + tabid).style.display = 'inline';
    }
}
//关闭当前tab
